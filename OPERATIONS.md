...
### 5. “매일 Slack 진행 알림이 안 온다”

- 정기 실행은 **로컬 작업 스케줄러**뿐이고, GitHub `schedule`은 없다. 클라우드에서 매일 도는 게 아니면 그쪽으로는 알림이 오지 않는다.
- Slack은 **수집 프로세스가 실행된 뒤** `send_slack_message`가 호출될 때만 간다. PC가 꺼져 있거나 작업이 스킵되면 메시지 자체가 없다.
- 확인 순서: `Get-ScheduledTask` / `scripts\status.ps1` → `logs\collector.log`에 `G2B 수집 시작` 유무 → `.env`의 Slack 변수.

### 6. 작업 스케줄러 `LastTaskResult`가 3221225786 등 비정상일 때

- `3221225786`(16진 `0xC000013A`)은 Windows에서 **프로세스가 Ctrl+C·강제 중단**으로 끊긴 경우 등에 나올 수 있다.
- `LastRunTime`은 오늘인데 `collector.log`에 **그 시각의 `G2B 수집 시작`이 없으면**, 배치가 로그를 남기기 전에 끊겼거나 콘솔/세션 문제일 수 있다.
- **조치:** 프로젝트 폴더에서 `run_collector.bat`를 수동 실행해 정상 완료되는지 본다. 계속 실패하면 작업 스케줄러의 **로그온 방식(Interactive 등)**·**다른 인스턴스**·전원 옵션을 확인한다.
- **참고:** 터미널에서 파이프로 출력을 자르면(`Select-Object -First` 등) 수집 프로세스가 **일찍 끊길 수 있으니** 일일 실행은 **항상 `run_collector.bat`(리다이렉트 `>> logs\collector.log`)**만 쓴다.

### 7. “수집 시작” 메시지만 오고 후속 알림이 없을 때
...
### 8. API 호출을 다 안 채웠는데 수집 완료 메시지가 올 때
