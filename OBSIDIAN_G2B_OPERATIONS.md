...
```
G2B 공공데이터 API (apis.data.go.kr)
        |
        |  HTTP 요청 (XML 응답, 999건/페이지)
        v
  collect_all.py / fill_gaps.py
        |
        |  XML 파싱 -> Python dict -> INSERT OR IGNORE
        v
  D:\pdeck-data\g2b-collector\g2b.db    <- SQLite (6GB, 175만건+)
        |
        |  S3 백업 (매일 수집 직후, run_collector.bat)
        v
  s3://zigu-data-lake/backup/g2b/g2b.db
        |
        |  주간 변환 (weekly_parquet_backup.py)
        v
  D:\pdeck-data\g2b-collector\parquet\contracts.parquet  (651MB)
```

- API -> 메모리 -> DB 직행. 중간 파일 없음
- 일일 API 한도: 1000회 (하루 최대 ~999,000건)
- DB는 외장하드(D:)에 저장. 환경변수 `G2B_DB_PATH`로 오버라이드 가능
- CSV 전체 덤프(`contracts_full.csv`, 4.2GB)도 외장하드에 있음
